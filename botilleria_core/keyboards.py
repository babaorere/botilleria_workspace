...
def get_categories_keyboard(categories: List[str]) -> InlineKeyboardMarkup:
    ...
    for i, category in enumerate(categories):
        ...
        row.append(
            InlineKeyboardButton(
                text=f"{emoji} {category}", callback_data=f"category:{category}"
            )
        )
    ...
    if categories:
        keyboard.append(
            [InlineKeyboardButton(text="📋 Ver todo", callback_data="category:all")]
        )
...
def get_products_keyboard(
    products: List[dict], page: int = 0, items_per_page: int = 5
) -> InlineKeyboardMarkup:
    """
    ...
    for product in page_products:
        ...
        button_text = (
            f"{stock_status} {product['name'][:20]}... - ${product['price']:,.0f}"
        )

        keyboard.append(
            [
                InlineKeyboardButton(
                    text=button_text, callback_data=f"product_detail:{product['id']}"
                ),
                InlineKeyboardButton(
                    text="[+]", callback_data=f"add_to_cart:{product['id']}"
                ),
            ]
        )
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="◀ Anterior", callback_data=f"products_page:{page - 1}"
            )
        )
    ...
    if page < total_pages - 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="Siguiente ▶", callback_data=f"products_page:{page + 1}"
            )
        )
...
def get_cart_items_keyboard(cart_items: List[dict]) -> InlineKeyboardMarkup:
    ...
    for item in cart_items:
        ...
        keyboard.append(
            [
                ...
                InlineKeyboardButton(
                    text="[−]", callback_data=f"cart_remove_one:{item['product_id']}"
                ),
                InlineKeyboardButton(
                ...
                InlineKeyboardButton(
                    text="[+]", callback_data=f"cart_add_one:{item['product_id']}"
                ),
                InlineKeyboardButton(
                    text="[🗑]", callback_data=f"cart_remove_all:{item['product_id']}"
                ),
            ]
        )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(text="🗑 Vaciar Carro", callback_data="cart_clear_all"),
            InlineKeyboardButton(text="🟢 PEDIR AHORA", callback_data="cart_checkout"),
        ]
    )
...
def get_order_method_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                text="🏪 RETIRO EN LOCAL\n(Gratis)", callback_data="method:retiro"
            ),
            InlineKeyboardButton(
                text="🚚 DESPACHO\n($2.500 / gratis > $30k)",
...
def get_order_confirmation_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                text="✅ CONFIRMAR PEDIDO", callback_data="order_confirm"
            ),
            InlineKeyboardButton(text="❌ CANCELAR", callback_data="order_cancel"),
        ]
    ]
    return InlineKeyboardMarkup(keyboard)
...
def get_pagination_keyboard(
    current_page: int, total_pages: int, prev_callback: str, next_callback: str
) -> InlineKeyboardMarkup:
    """
    ...
    if current_page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="◀ Anterior", callback_data=f"{prev_callback}:{current_page - 1}"
            )
        )
    ...
    if current_page < total_pages - 1:
        nav_buttons.append(
            InlineKeyboardButton(
                text="Siguiente ▶", callback_data=f"{next_callback}:{current_page + 1}"
            )
        )
...
def get_profile_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                text="📝 Editar Nombre", callback_data="profile_edit:name"
            ),
            InlineKeyboardButton(
                text="📧 Editar Email", callback_data="profile_edit:email"
            ),
        ],
        [
            InlineKeyboardButton(
                text="🆔 Editar RUT", callback_data="profile_edit:rut"
            ),
            InlineKeyboardButton(
                text="📍 Editar Dirección", callback_data="profile_edit:address"
            ),
        ],
        [
            InlineKeyboardButton(
                text="📍 + Agregar Dirección", callback_data="profile_add_address"
            ),
            InlineKeyboardButton(
                text="🔔 Notificaciones", callback_data="profile_toggle_notifications"
            ),
        ],
        [
            InlineKeyboardButton(
                text="🗑 Eliminar Cuenta", callback_data="profile_delete"
            ),
            InlineKeyboardButton(
                text="🔙 Volver al Menú", callback_data="profile_back_to_main"
            ),
        ],
    ]
    return InlineKeyboardMarkup(keyboard)
...
def get_order_history_keyboard(
    orders: List[dict], page: int = 0, items_per_page: int = 5
) -> InlineKeyboardMarkup:
    """
    ...
    for order in page_orders:
        ...
        keyboard.append(
            [
                ...
                InlineKeyboardButton(
                    text="[🔁] Repetir", callback_data=f"order_repeat:{order['id']}"
                )
                if order.get("status") in ["entregado", "preparando"]
    ...
    if total_pages > 1:
        ...
        if page > 0:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="◀ Anterior", callback_data=f"history_page:{page - 1}"
                )
            )

        nav_buttons.append(
            InlineKeyboardButton(
                text=f"Pág {page + 1}/{total_pages}", callback_data="page_info"
            )
        )
        ...
        if page < total_pages - 1:
            nav_buttons.append(
                InlineKeyboardButton(
                    text="Siguiente ▶", callback_data=f"history_page:{page + 1}"
                )
            )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                text="📋 Ver Todo (últimos 20)", callback_data="history_view_all"
            ),
            InlineKeyboardButton(
                text="🔙 Volver", callback_data="history_back_to_main"
            ),
        ]
    )
...
def get_empty_state_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                text="🔍 Explorar Catálogo", callback_data="browse_categories"
            ),
        ...
        [
            ...
            InlineKeyboardButton(
                text="🤝 Contactar Humano", callback_data="contact_human"
            ),
        ],
    ]
    return InlineKeyboardMarkup(keyboard)
