...
def check_abandoned_carts(minutes_threshold: int = 30):
    ...
    """
    logger.info(
        "Iniciando escaneo de carritos abandonados (más de %d mins)...",
        minutes_threshold,
    )
    from datetime import timezone

    threshold_time = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(
    ...
    with SessionLocal() as db:
        ...
        stale_items = (
            db.query(CartItem).filter(CartItem.updated_at < threshold_time).all()
        )
        ...
        for session_id, data in carts_by_session.items():
            ...
            if tenant:
                ...
                if bot_token:
                    ...
                    if success:
                        logger.info(
                            "Mensaje de recuperación enviado a %s exitosamente.",
                            session_id,
                        )
                    else:
                        logger.error(
                            "Fallo al enviar mensaje de recuperación a %s.", session_id
                        )
                else:
                    logger.warning(
                        "No hay token de Telegram configurado para el tenant %s.",
                        tenant.name,
                    )
