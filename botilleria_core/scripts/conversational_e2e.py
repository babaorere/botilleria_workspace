...
def run_e2e():
    ...
    turns = [
        ...
        (
            "Hola, buenas. ¿Qué cervezas tienen disponibles y a qué precio?",
            "Turno 2: Consulta de catálogo",
        ),
        (
            "Perfecto. Agrega 6 cervezas Corona al carrito porfa.",
            "Turno 3: Agregar al carrito",
        ),
        (
            "¿Tienen alguna promo o algo para comer?",
            "Turno 4: Consulta de cross-selling / snacks",
        ),
        (
            "Ya, genial. Agrega también 1 Papas Fritas Lays y muestra mi carrito.",
            "Turno 5: Agregar snack y ver carrito",
        ),
        ("Excelente. Quiero confirmar el pedido.", "Turno 6: Solicitar confirmación"),
        (
            "Soy Juan Pérez, mi teléfono es +56912345678, y quiero despacho a Av. Vitacura 1234.",
            "Turno 7: Proporcionar datos de confirmación",
        ),
    ]
    ...
    for message, label in turns:
        ...
        print(
            f"🤖 Asistente (Latencia: {latency:.2f}s, Estado: {data.get('state')}, Versión: {data.get('version')}):"
        )
        ...
        if message == "Perfecto. Agrega 6 cervezas Corona al carrito porfa.":
            ...
            assert items[0].quantity == 6, (
                "La cantidad de Cerveza Corona debería ser 6."
            )
            ...
            items = check_cart(session_id)
            assert len(items) == 0, (
                "El carrito debería estar vacío tras confirmar el pedido."
            )
