...
def run_scenario_1_support():
    ...
    send_chat_message(session_id, user_id, "¿Dónde están ubicados y hasta qué hora atienden?")
    
    # 3. Preguntar por producto inexistente
    send_chat_message(session_id, user_id, "¿Tienen whisky azul etiqueta dorada de 50 años?")
...
def run_scenario_2_cancellation():
    ...
    # 1. Agregar productos
    send_chat_message(session_id, user_id, "Hola, por favor agrega 3 cervezas Heineken al carro.")
    time.sleep(1)
    ...
    send_chat_message(session_id, user_id, "No, mejor ya no quiero comprar nada. Cancela todo y vacía mi carro.")
    time.sleep(1)
...
def run_scenario_3_changes():
    ...
    # 1. Agregar Kunstmann
    send_chat_message(session_id, user_id, "Buenas, agrégame 2 cervezas Kunstmann Torobayo al carro.")
    time.sleep(1)
    ...
    send_chat_message(session_id, user_id, "Oye, mejor cambia la cantidad a 4 de esas Kunstmann por favor. Limpia el carro primero y pon las 4.")
    time.sleep(1)
    ...
    assert items[0]["quantity"] == 4, "La cantidad de Kunstmann debería haberse actualizado a 4."
...
def run_scenario_4_abuse():
    ...
    send_chat_message(session_id, user_id, "No me sirves para nada inútil de mierda, pásame con alguien de verdad.")
